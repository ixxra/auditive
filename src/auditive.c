...
static gboolean auditive_rememberable_screen (auditive* self, gint key_num, gint hot_key, world_widescreen_type screen_type) {
	...
	_tmp14_ = screen_type;
	_tmp15_ = auditive_index_screen (self, _tmp14_);
	if (_tmp15_ == NULL) {
		result = TRUE;
		return result;
	}
	...
	console_cls (_tmp17_);
	_tmp19_ = self->world;
	_tmp20_ = _tmp19_->screen_current;
	_tmp21_ = screen_type;
	if (_tmp20_ == _tmp21_) {
		world_widescreen_type _tmp22_;
		screen* _tmp23_ = NULL;
		world_widescreen_type _tmp24_;
		_tmp22_ = screen_type;
		_tmp23_ = auditive_index_screen (self, _tmp22_);
		_tmp24_ = _tmp23_->last_screen;
		_tmp18_ = _tmp24_ != WORLD_WIDE_SCREEN_TYPE_none;
	} else {
		_tmp18_ = FALSE;
	}
	_tmp25_ = _tmp18_;
	if (_tmp25_) {
		world_widescreen_type _tmp26_;
		_tmp26_ = screen_type;
		auditive_restore_screen (self, _tmp26_);
	} else {
		world_widescreen_type _tmp27_;
		_tmp27_ = screen_type;
		auditive_remember_screen (self, _tmp27_);
	}
	result = TRUE;
	return result;
}
...
static gboolean auditive_key_callback (auditive* self, GIOChannel* src, GIOCondition cond) {
	...
	switch (_tmp13_) {
		...
		{
			world_wide* _tmp18_;
			world_wideplaying_type _tmp19_;
			_tmp18_ = self->world;
			_tmp19_ = _tmp18_->playing_state;
			switch (_tmp19_) {
				case WORLD_WIDE_PLAYING_TYPE_play:
				{
					gint64 _tmp20_ = 0LL;
					_tmp20_ = auditive_get_position (self);
					self->priv->resume_position = _tmp20_;
					auditive_pause (self);
					break;
				}
				case WORLD_WIDE_PLAYING_TYPE_pause:
				{
					...
					_tmp22_ = self->priv->resume_position;
					gst_element_seek_simple (_tmp21_, GST_FORMAT_TIME, GST_SEEK_FLAG_FLUSH | GST_SEEK_FLAG_KEY_UNIT, _tmp22_);
					auditive_playing (self);
					break;
				}
				default:
		...
		case '-':
		{
			...
			_tmp23_ = self->world;
			_tmp24_ = self->world;
			_tmp25_ = _tmp24_->play_advance;
			_tmp23_->play_advance = !_tmp25_;
			auditive_draw (self);
			break;
		}
		default:
		{
			...
			gint _tmp29_;
			_tmp26_ = self->world;
			_tmp27_ = _tmp26_->screen_current;
			_tmp28_ = auditive_index_screen (self, _tmp27_);
			_tmp29_ = key_num;
			screen_key_press (_tmp28_, _tmp29_);
			break;
		}
	}
	result = TRUE;
...
static gboolean auditive_bus_callback (auditive* self, GstBus* bus, GstMessage* message) {
	...
	switch (_tmp1_) {
		...
		case GST_MESSAGE_TAG:
		{
			...
			tag_list = _tmp15_;
			_tmp16_ = tag_list;
			taglist_foreach_job (_tmp16_, _auditive_tag_reciever_foreach_delegate, self, self);
			_gst_tag_list_free0 (tag_list);
			break;
		}
		default:
