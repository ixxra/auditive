/* screen.c generated by valac 0.20.1, the Vala compiler
	...
	switch (_tmp6_) {
		case CONSOLE_key_up:
		{
			...
			if (_tmp7_ > 0) {
				gint _tmp8_;
				_tmp8_ = self->row_pos;
				self->row_pos = _tmp8_ - 1;
			} else {
				gint _tmp9_;
				_tmp9_ = self->screen_pos;
				if (_tmp9_ > 0) {
					gint _tmp10_;
					_tmp10_ = self->screen_pos;
					self->screen_pos = _tmp10_ - 1;
				}
			}
			screen_draw (self);
			break;
		...
		case CONSOLE_key_down:
		{
			...
			_tmp11_ = index;
			_tmp12_ = size;
			if ((_tmp11_ + 1) >= ((guint) _tmp12_)) {
				break;
			}
			_tmp13_ = self->row_pos;
			_tmp14_ = rows;
			if (_tmp13_ < (_tmp14_ - 1)) {
				gint _tmp15_;
				_tmp15_ = self->row_pos;
				self->row_pos = _tmp15_ + 1;
			} else {
				gint _tmp16_;
				_tmp16_ = self->screen_pos;
				self->screen_pos = _tmp16_ + 1;
			}
			screen_draw (self);
			break;
		}
		...
		case CONSOLE_key_ppage:
		{
			...
			gint _tmp18_;
			gint _tmp19_;
			_tmp17_ = self->screen_pos;
			_tmp18_ = rows;
			self->screen_pos = _tmp17_ - _tmp18_;
			_tmp19_ = self->screen_pos;
			if (_tmp19_ < 0) {
				self->screen_pos = 0;
				self->row_pos = 0;
			}
		...
		case CONSOLE_key_npage:
		{
			...
			_tmp21_ = rows;
			self->screen_pos = _tmp20_ + _tmp21_;
			_tmp22_ = self->screen_pos;
			_tmp23_ = rows;
			_tmp24_ = size;
...
static void screen_finalize (screen* obj) {
	...
	world_wide* _tmp0_;
	screen* _tmp1_;
	self = G_TYPE_CHECK_INSTANCE_CAST (obj, TYPE_SCREEN, screen);
	_tmp0_ = self->world;
